...
public class central_ : MonoBehaviour {
	...
	void Start(){
		...
		if(idLenguaje == 0) nivelLabel.text = "Level " + nivelActual;
		if(idLenguaje == 1) nivelLabel.text = "Nivel " + nivelActual;
		if(nivelActual == 1){
		...
		if(idLenguaje == 0) mostrarMensaje("Ready?");
		if(idLenguaje == 1) mostrarMensaje("Listo?");
		...
		if(monstruosNivel!=null && monstruosNivel.Length > 0){
			...
			for(int i = ind; i < ind + estaciones; i++){
				if(monstruosNivel[i - ind] == null){
					...
					print ("monstruo creado en "+(i-ind));
				}
				else{
			...
			for(int i = ind; i < ind + estaciones; i++){
				...
				PlayerPrefs.SetInt("asiento"+(i - ind), monstruosDesbloqueadosID[Random.Range(0, nMonstruosDesbloqueados)]);
				Transform t = (Transform) Instantiate(monstruos[PlayerPrefs.GetInt("asiento" + (i - ind) , 0) * 5 + globalVariables.monstruosDificultadNivel[i]], new Vector3(1000 * (i - ind), -1000, 150), Quaternion.identity);
				GameObject g = t.gameObject;
				monstruosNivel[i - ind] = g.GetComponent<monstruo>();
				monstruosNivel[i - ind].componer();
	...
	void Update () {
		...
		case 0:
			if(Time.timeSinceLevelLoad > 1.5){ 
				...
				if(idLenguaje == 0) mostrarMensaje("GO!");
				if(idLenguaje == 1) mostrarMensaje("VAMOS!");
				//for(int i = 0; i < monstruosNivel.Length; i++)
				//	monstruosNivel[i].iniciar();
	...
	void setTiempo(){
		...
		string tiempoActual = (System.DateTime.Parse(t) - System.DateTime.Now).ToString().Substring(0, 8);
	...
	IEnumerator perdiste(){
		if(!mostrandoPerdiste){
			...
			if(peluqueroNivel.vidas - 1 < 0){
				...
				string mensajeFinal;
				if(PlayerPrefs.GetInt("enChallenge", 0) == 2){
					mensajeFinal = "Challenge Passed!";
					botonFacebookPublicar.SendMessage("setMensaje", "I had defeated a barbershop with " + PlayerPrefs.GetInt("challengeStartScore") + " points. Can you beat me?");
				}
				else{
					mensajeFinal = "HAIR GOD!";
					if(puntajeNivel < 200) mensajeFinal = "Not bad";
					else 
						if(puntajeNivel < 500) mensajeFinal = "Good!";
						else 
							if(puntajeNivel < 1500) mensajeFinal = "Well Done!";
							else 
								if(puntajeNivel < 3000) mensajeFinal = "Great performance!";
				...
				if(idLenguaje == 0) mostrarMensaje(mensajeFinal);
				if(idLenguaje == 1) mostrarMensaje("Juego Terminado");
				yield return new WaitForSeconds(2);
				...
				gameFinishedScript.setInformacion(tiempoTermino ,puntajeTotal + puntajeNivel, clientesAtendidos, (int)expCollected, clientesAtendidos * 2 * mult);
				...
				int[] valores = {hairCuts, topHairCuts, clientesAtendidos, PlayerPrefs.GetInt("peluqueroSeleccionado", 0) == 0?poder1usos:0, PlayerPrefs.GetInt("peluqueroSeleccionado", 0) == 0?poder2usos:0, 
								...
								PlayerPrefs.GetInt("peluqueroSeleccionado", 0) == 0?poder1usos:0, PlayerPrefs.GetInt("peluqueroSeleccionado", 0) == 0?poder2usos:0, (int)tiempoTermino, useSpecialScissor?1:0, (useVisualPel1||useVisualPel2||useVisualPel3||PlayerPrefs.GetInt("peluqueria", 0) > 0)?1:0,
								...
								PlayerPrefs.GetInt("objeto2oro", 0) > 0?1:0, useVisualPel1?1:0, PlayerPrefs.GetInt("peluqueroSeleccionado", 0) == 2?1:0, PlayerPrefs.GetInt("peluqueroSeleccionado", 0) == 2?poder1usos:0, PlayerPrefs.GetInt("peluqueroSeleccionado", 0) == 2?poder2usos:0,
								...
								(int)tiempoTermino, vidasUsadas > 1?1:0, useVisualPel2?1:0, hairCuts, vidasUsadas > 2?1:0,
								PlayerPrefs.GetInt("peluqueroSeleccionado", 0) == 2?poder1usos:0, PlayerPrefs.GetInt("peluqueroSeleccionado", 0) == 2?poder2usos:0, useVisualPel3?1:0, PlayerPrefs.GetInt("objeto3oro", 0) > 0?1:0, PlayerPrefs.GetInt("peluqueroSeleccionado", 0) == 1?poder1usos:0,
				...
				if(idLenguaje == 0) mostrarMensaje("EXTRA LIFE!");
				if(idLenguaje == 1) mostrarMensaje("VIDA EXTRA!");
